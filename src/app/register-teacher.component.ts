...
import {Teacher, TeacherCreate} from "./model/teacher";
import {ToasterService} from "angular2-toaster";

@Component({
  //to indicate that html and css are relative to the component
  template: `
    <h3>Maak een account aan als gastdocent</h3>
    <form class="col-lg-3" [formGroup]="mainForm">
      ...
      <div class="form-group row mt-3">
        ...
        <div class="col-sm-10">
          ...
          <div
            *ngIf="getControl['password'].touched && getControl['password_repeat'].touched && (getControl['password'].invalid || getControl['password_repeat'].invalid)"
            class="text-danger">
            <div>Wachtwoorden moet minimaal 8 tekens lang zijn</div>
          </div>
          <div
            *ngIf="getControl['password'].touched && getControl['password_repeat'].touched && (getControl['password'].value != getControl['password_repeat'].value)"
            class="text-danger">
...
export class RegisterTeacherComponent {
  ...
  mainForm: FormGroup;
  create: TeacherCreate = new TeacherCreate();

  constructor(private teacherService: TeacherService,
              private eb: EventBus,
              private fb: FormBuilder) {

    eb.on(Teacher.CREATED_EVENT, teacher => {
  ...
  register() {
    ...
    this.teacherService.create(this.create)
  }
